...
server_ip = '34.254.177.17'
server_port = '5672'
virtual_host = 'dar-tanks'
credentials = pika.PlainCredentials(username = 'dar-tanks', password ='5orPLExUYnyVYZg48caMpX')
...
def duel():
    ...
    while mainloop:
        ...
        if (booster1.x + 23 > tank1.x + 9 > booster1.x ) and (booster1.y + 33> tank1.y + 9 > booster1.y) and booster1.status == True:
            booster1.status = False
            timer1 = True
        if (booster1.x + 23 > tank2.x + 9 > booster1.x ) and (booster1.y + 33> tank2.y + 9 >booster1.y) and booster1.status == True:
            booster1.status = False
            timer2 = True
            if timer1:
...
def multiplayer():
    ...
    while AImode:
        screen = pygame.display.set_mode((width, height))
        screen.fill((127,255,212))
        pygame.display.set_caption("MultiplayerAI")
        pygame.display.flip()
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                quit()    
